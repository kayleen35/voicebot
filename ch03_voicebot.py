...
def main():
    ...
    with st.expander("음성비서 프로그램에 관하여", expanded=True):
        ...
        - TTS(Text-To-Speech)는 구글의 Google Trnaslate TTS를 활용했습니다.
        """
        )
    ...
    with col2:
        ...
        if (audio.duration_seconds > 0) and (st.session_state["check_audio"]==False):
            ...
            for sender, time, message in st.session_state["chat"]:
                if sender == "user":
                    st.write(f'<div style="display:flex;align-items:center;"><divstyle="background-color:#007AFF; color:white;border-radius: 12px;padding:8px 12px;margin-right:8px;">{message}</div><div style="font-size:0.8rem;color:gray;">{time}</div></div>', unsafe_allow_html=True)
                    st.write("")
                else:
